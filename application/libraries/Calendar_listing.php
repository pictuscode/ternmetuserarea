...
class CalendarObj {
  ...
  #var $spl_date_array=array(array('date'=>'2018-04-25','spl_price'=>'$2500','status'=>'available'),array('date'=>'2018-02-26','spl_price'=>'$4500','status'=>'booked','s1.png'));
  var $spl_date_array=array();
  ...
  function __construct($year=false, $month=1, $day=1, $hour=0, $minute=0, $second=0) {
    ...
    $this->secondINT = intval($second); #echo $year.'~'.$month.'!'.$day.'<br/>';
	$date_now=$year.'-'.$month.'-'.$day;	
    $this->date_array=$_SESSION['date_array']; 
    $this->spl_date_array=$_SESSION['spl_date_array']; 
    #$this->date_array=array("0"=>"2018-03-03");
    if(in_array((string)$date_now,$this->date_array)){
	    $this->status = 1;
	    $this->title = "Blocked";
		$this->image = "";
		$this->price = "";
	...
	else
	{
		...
		$this->title = "";
		$this->image = "";	
		$this->price = $_SESSION['dailyprice'];
	}		
    foreach($this->spl_date_array as $darray){
    if((string)$date_now==$darray["date"] && $darray['status']==2){
	    $this->status = 2;
	    $this->title = "Available";
		$this->image = "";
		$this->price = $darray['spl_price'];
	}
	else if((string)$date_now==$darray["date"] && $darray['status']==3){
	
		$this->status = 3;
		$this->title = "Booked By ".$darray['fname'];
		$this->image = $darray['image'];
		$this->bid = $darray['bid'];
		$this->booking_id = $darray['booking_id'];
		$this->price = $darray['spl_price'];
	
	}
	else if((string)$date_now==$darray["date"] && $darray['status']==1)
	{
		$this->status = 1;
	    $this->title = "Blocked";
		$this->image = "";
