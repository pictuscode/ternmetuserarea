...
class CalendarObj {
  ...
  #var $spl_date_array=array(array('date'=>'2018-02-25','spl_price'=>'$2500','status'=>'available'),array('date'=>'2018-02-26','spl_price'=>'$4500','status'=>'booked','s1.png'));
  ...
  function __construct($year=false, $month=1, $day=1, $hour=0, $minute=0, $second=0) {
   ...
   /*  if((string)$date_now==$darray["date"] && $darray['status']==2){
		...
		$this->status = 3;
		$this->title = "Booked By siva";
		$this->image = "";
		$this->price = $darray['spl_price'];
	
	}
	else */ 
	if((string)$date_now==$darray["date"] && $darray['status']==1)
	{
		$this->status = 1;
	    $this->title = "Blocked";
		$this->image = "";
