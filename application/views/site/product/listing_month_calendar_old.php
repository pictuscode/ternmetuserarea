...
<p>  <?php if(date('Y-m',strtotime($prevMonth->year()->int().'-'.$prevMonth->int().'-'."01"))>=date("Y-m") ) {?> <span> <a id="nav_prev" class="arrow" href="javascript:void(0);" data-value="<?php echo $prevMonthURL ?>">&larr;</a> </span> <?php } ?> 
  ...
  <table id="beforepublish">
    ...
    <tr>  
	  ...
	  ?>
      <td data-id="<?php echo $i;?>" data-value="<?php echo $currentMonth->year()->int().'-'.$mon."-".$days;?>" data-format="<?php echo $days."-".$mon."-".$currentMonth->year()->int();?>" id="<?php echo $currentMonth->year()->int().'-'.$mon."-".$days;?>" class="data_id<?php echo $i;?><?php if($day->status==1){ echo " block";}?><?php if($day->status==3){ echo " uibooked";}?><?php if(date('Y-m-d',strtotime($currentMonth->year()->int().'-'.$currentMonth->int().'-'. $day->int()))<date("Y-m-d")){ echo " past_date";}?><?php if($day->month() != $currentMonth) echo 'inactive' ?>" title="<?php echo $day->title;?>"> <div class="date_day"> <?php echo ($day->isToday()) ? '<strong>' . $day->int() . '</strong>' : $day->int() ?> </div> <div class="date_price"> <label><?php echo $day->price;?></label></div>
