...
$(document).on("click",".delete_photo",function(){
	...
	$.post(baseurl+'site/product/delete_img',{'pid':$(".listing_base").attr('data-id'),'fname':fname},function(data){
...
$(document).on("click",".delete_photo_cover",function(){ 
	...
	var lival=$(this);
	$(this).addClass("activebg");
	$.post(baseurl+'site/product/delete_photo_cover',{'pid':$(".listing_base").attr('data-id'),'fname':fname},function(data){ data = JSON.parse(data);
		if(data.coverimg!=""){
...
function sortbyli()
{
	...
	$.post(baseurl+"site/product/imgsort",{'pid':$(".listing_base").attr('data-id'),'imgnames':imgarry},function(data){})
}
$(document).ready(function(){
	load_photo();
});
...
function load_photo(){
	...
	$.post(baseurl+"site/product/getphotos",{'pid':$(".listing_base").attr('data-id')},function(data){  $("#photo_tab").html(data); 
	    $('.photo_preview_ul').sortable({
		items: "li:not(.last_photo_li)",
		connectWith: '.upload_images_base',
		update: function( event, ui ) { sortbyli();}
...
function loadroom_tab(){
	...
	$.post(baseurl+"site/product/getroom",{'pid':$(".listing_base").attr('data-id')},function(data){ $("#bedroom").html(data); 
	load_listing_rooms();    
	})
}

function loadamenties_tab(){	
	$.post(baseurl+"site/product/get_amenities",{'pid':$(".listing_base").attr('data-id')},function(data){ $("#amenities").html(data); 
	$(".bottom_next_continue").hide();
...
function loadhouserule_tab(){
	$.post(baseurl+"site/product/ajax_rules",{'pid':$(".listing_base").attr('data-id')},function(data){ $("#houserules").html(data); 
    $(".bottom_next_continue").hide();
...
function loadbooking_tab(){
	$.post(baseurl+"site/product/ajax_bookings",{'pid':$(".listing_base").attr('data-id')},function(data){ $("#booking_price").html(data); 
      $(".bottom_next_continue").hide();
	  $(".booking_tab_bottom").show();
	  $.post(baseurl+"site/product/ajax_cal",{'pid':$(".listing_base").attr('data-id')},function(data){ $("#calender_load").html(data); 
...
{      
       $.post(baseurl+"site/product/getlocation",{'pid':$(".listing_base").attr('data-id')},function(data){ $("#location").html(data); 
	     $(".bottom_next_continue").hide();
...
function init_map() { 
  ...
  autocomplete = new google.maps.places.Autocomplete(
	...
	google.maps.event.addListener(autocomplete, 'place_changed', function() {
		...
		$.post(baseurl+"site/product/get_location",{'address':data,'pid':$(".listing_base").attr('data-id'),'country_id':country_id},function(data){ var data=JSON.parse(data);
...
$(document).on("click","#map_save",function(){
	...
	$.post(baseurl+"site/product/dragsave_location",{'pid':$(".listing_base").attr('data-id'),'lat':lat,'lng':lng},function(data){ var data=JSON.parse(data);
			$("#autocomplete_street_address").val(data.street);
			$("#address_city").val(data.city);
			$("#address_state").val(data.state);
			$("#address_zipcode").val(data.zipcode);
...
$(document).on("change",".save_function",function(){   
   ...
   $.post(baseurl+"site/product/save_listing_info",{'pid':$(".listing_base").attr('data-id'),'colum_name':colum_name,'colum_value':colum_value},function(data){ }) 
});
...
$(document).on("change",".save_function_multiple_checkbox",function(){   
   ...
   var colum_value=$(this).val();   
   $.post(baseurl+"site/product/save_listing_info_multiple_checkbox",{'pid':$(".listing_base").attr('data-id'),'colum_name':colum_name.replace('[]',''),'colum_value':resultarray},function(data){ }) 
...
$(document).on("change","#list_type",function(){   
   ...
   if(list_type_id==""){
	   ...
	   $.post(baseurl+"site/product/load_listing_property_type",{'pid':$(".listing_base").attr('data-id'),'list_type_id':list_type_id},function(data){
		   ...
		   {   
			   ...
			   if(cur_count!=0)
			   {
				   ...
				   setTimeout(function(){
					...
					$.post(baseurl+"site/product/remove_bedrooms",{'pid':$(".listing_base").attr('data-id'),'remove_bedrooms':'br'+cur_count,'beds_count':total_beds,'bathroom_count':total_bedrooms},function(data){ })
					},2000);	
				   }	   
...
$(document).on("click",".remove_done_btn",function() {
		...
		var total_beds=get_bed_counts(); 
		var total_bedrooms=get_bathrooms_counts(); 
		$.post(baseurl+"site/product/remove_bedrooms",{'pid':$(".listing_base").attr('data-id'),'remove_bedrooms':'br'+remove_id,'beds_count':total_beds,'bathroom_count':total_bedrooms},function(data){ })
		},2000);		
});
...
$(document).on("click",".bed_countingbox_plus",function(){   
   ...
   var total_beds=get_bed_counts();
   var total_bedrooms=get_bathrooms_counts();    
   $.post(baseurl+"site/product/save_bed_info",{'pid':$(".listing_base").attr('data-id'),'bed_type_id':bed_type_id,'count':ex_count,"bedroom_no":bedroom_no,'beds_count':total_beds,'bathroom_count':total_bedrooms},function(data){ }) 
...
$(document).on("click",".pbcheckbox_new",function(){   
   ...
   $.post(baseurl+"site/product/save_bed_info",{'pid':$(".listing_base").attr('data-id'),'bed_type_id':bed_type_id,'count':a,"bedroom_no":bedroom_no,'beds_count':total_beds,'bathroom_count':total_bedrooms},function(data){ })
...
$(document).on("click",".common_remove_done_btn",function() {
		...
		var total_beds=get_bed_counts(); 
		var total_bedrooms=get_bathrooms_counts(); 
		$.post(baseurl+"site/product/remove_bedrooms",{'pid':$(".listing_base").attr('data-id'),'remove_bedrooms':'cbr','beds_count':total_beds,'bathroom_count':total_bedrooms},function(data){ })
		},2000);		
});
...
$(document).on("click","#add_rule_btn",function(){   
   ...
   if(add_rule_text!=""){
   $.post(baseurl+"site/product/add_rules",{'pid':$(".listing_base").attr('data-id'),'add_rule_text':add_rule_text,'ruleno':maxval},function(data){ 
   var data=JSON.parse(data);
	if(data.status==1)
	{
...
$(document).on("click",".delete_new_rules_btn",function(){   
  ...
  var par=$(this).parent();
   $.post(baseurl+"site/product/delete_rules",{'pid':$(".listing_base").attr('data-id'),'remove_id':$(this).attr("data-id")},function(data){ 
   var data=JSON.parse(data);
	if(data.status==1)
	{
		par.remove();
...
$(document).ready(function(){
	$(document).on('click','#beforepublish td',function(){
		if($(this).hasClass("block") && !$(this).hasClass("inactive")){
		var val=$(this).attr("data-value"); 
		$(this).removeClass("block");
		$.post(baseurl+"site/product/remove_block_date",{'pid':$(".listing_base").attr('data-id'),'block_dates':val},function(data){ 
		...
		$(this).addClass("block");
		var val=$(this).attr("data-value"); 
        $.post(baseurl+"site/product/save_block_date",{'pid':$(".listing_base").attr('data-id'),'block_dates':val},function(data){ 
...
var cal_status="";	
    ...
    $(document).on("mousedown","td",function() {
		...
		/* if(parseInt(date_array_saveid[date_array_saveid.length-1])>=parseInt(stop))
		{  
			for(var i=parseInt(date_array_saveid[date_array_saveid.length-1]);i>=parseInt(stop);i--)
			{
			        $('.data_id'+i).removeClass("block");	
			        $('.data_id'+i).removeClass("current_click");	
				...
				{ 
					 ...
					 date_array_remove=[];			 
						$(".remove_click").each(function(i){
							if($.inArray($(this).attr("data-value"), date_array_remove) == -1){
							 date_array_remove.push($(this).attr("data-value")); 
						   }
						});
						cal_status=0;
				}
				else
				{
					 ...
					 $('.data_id'+i).addClass("current_click");	
						...
						$(".current_click").each(function(i){
							if($.inArray($(this).attr("data-value"), date_array_save) == -1){
							 date_array_save.push($(this).attr("data-value")); 
							 date_array_saveid.push($(this).attr("data-id")); 
						   }
						});
						cal_status=1;
		...
		$(document).on("mouseup","td",function() {
			 ...
			 $(".current_click").each(function(i){
							if($.inArray($(this).attr("data-value"), date_array_save_final) == -1){
			 ...
			 if(date_array_save_final.length!=0){
				 ...
				 $.post(baseurl+"site/product/save_block_date_multiple",{'pid':$(".listing_base").attr('data-id'),'block_dates':date_array_save_final},function(data){ 
							date_array_save_final=[];			
							date_array_save=[];			
							date_array_saveid=[];			
									})
				}
				if(date_array_remove.length!=0){
				  $('td').removeClass("remove_click");	
				  $.ajaxSetup({async: true});  
				 $.post(baseurl+"site/product/remove_block_date_multi",{'pid':$(".listing_base").attr('data-id'),'block_dates':date_array_remove},function(data){ 
								date_array_remove=[];		
									})
				} 
			})	
...
$(document).on("click",".publish_btn",function(){
	 $.post(baseurl+"site/product/product_publish",{'pid':$(".listing_base").attr('data-id')},function(data){ 
	var data=JSON.parse(data);
	if(data.status==1)
	{
